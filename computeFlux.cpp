...
void Solver::computeFlux_y()
{
    ...
    for (int i = 0; i < ncx; i++)
    {
        for (int j = 0; j < f.nfy; j++)
        {
            ...
            // viscous part
            vy = vis_y(i, j);
            f.Fy[i][j][2] += (vy[0] * ni + vy[2] * nj) * A;
            f.Fy[i][j][3] += (vy[2] * ni + vy[1] * nj) * A;
            f.Fy[i][j][4] += (vy[3] * ni + vy[4] * nj) * A;
#endif
        }
    }
...
void Solver::computeFlux_x() {
    ...
    for (int j = 0; j < ncy; j++)
    {
        for (int i = 0; i < f.nfx; i++)
        {
            ...
            // viscous part
            vx = vis_x(i, j);
            f.Fx[i][j][2] += (vx[0] * ni + vx[2] * nj) * A;
            f.Fx[i][j][3] += (vx[2] * ni + vx[1] * nj) * A;
            f.Fx[i][j][4] += (vx[3] * ni + vx[4] * nj) * A;
#endif
        }
    }
